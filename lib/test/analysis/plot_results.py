...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    """plot_opts 中可含 show_legend / show_xlabel / show_ylabel / show_title（bool，默认均为 False）。"""
    plt.rcParams['text.usetex'] = False
    # Linux 通常无 Times New Roman；优先用 Matplotlib 自带的 DejaVu Serif，避免 findfont 警告
    plt.rcParams['font.family'] = 'serif'
    plt.rcParams['font.serif'] = ['DejaVu Serif', 'Liberation Serif', 'Nimbus Roman', 'Times New Roman', 'Times']
    # Plot settings
    ...
    # 默认不显示图例、坐标轴标题与图标题；需要时在 plot_opts 中设 show_legend/show_xlabel/show_ylabel/show_title 为 True
    show_legend = plot_opts.get('show_legend', False)
    show_xlabel = plot_opts.get('show_xlabel', False)
    show_ylabel = plot_opts.get('show_ylabel', False)
    show_title = plot_opts.get('show_title', False)
...
def plot_results(trackers, dataset, report_name, merge_results=False,
                 plot_types=('success'), force_evaluation=False, plot_style=None, **kwargs):
    """
    ...
    args:
        ...
        plot_style - 可选 dict，合并进各类曲线的 plot_opts，用于控制显示，例如：
                    show_legend, show_xlabel, show_ylabel, show_title（均为 bool，默认 False）
    """
    # Load data
    settings = env_settings()
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             'ylabel': 'Overlap Precision [%]', 'xlim': (0, 1.0), 'ylim': (0, 98), 'title': 'Success'}
        success_plot_opts.update(plot_style)
    ...
    if 'prec' in plot_types:
        ...
        precision_plot_opts.update(plot_style)
    ...
    if 'norm_prec' in plot_types:
        ...
        norm_precision_plot_opts = {'plot_type': 'norm_precision', 'legend_loc': 'lower right',
                                    'xlabel': 'Location error threshold', 'ylabel': 'Distance Precision [%]',
                                    'xlim': (0, 0.5), 'ylim': (0, 98), 'title': 'Normalized Precision'}
        norm_precision_plot_opts.update(plot_style)
    ...
    # plt.show()  # 无图形界面环境可保持注释，避免阻塞；本地交互查看时再取消注释


def generate_formatted_report(row_labels, scores, table_name='', show_header=True):
    """show_header=False 时不打印首行（数据集名 + AUC/OP50 等列名），只保留各 tracker 数据行。"""
    min_score_width = 10
...
def print_results(trackers, dataset, report_name, merge_results=False,
                  plot_types=('success'), show_table_header=True, **kwargs):
    ...
    args:
        ...
        show_table_header - 若为 False，不打印首行表头（数据集名与各指标列名），仅打印各 tracker 行
    """
    # Load pre-computed results
    ...
    report_text = generate_formatted_report(
        tracker_disp_names, scores, table_name=report_name, show_header=show_table_header)
    print(report_text)
