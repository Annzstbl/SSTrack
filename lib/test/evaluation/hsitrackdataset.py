...
class HSITrackDataset(BaseDataset):
    """ 
    HSITrack dataset.
    """
    def __init__(self, split):
        ...
        if split == 'test':
            self.base_path = os.path.join(self.env_settings.hsitrack_path, split)
        else:
            self.base_path = os.path.join(self.env_settings.hsitrack_path, 'train')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        return Sequence(sequence_name, frames_list, 'hsitrack', ground_truth_rect.reshape(-1, 4))
