...
#         im = np.concatenate((im1[:,:,2:3], im1[:,:,1:2], im1[:,:,0:1], im2[:,:,2:3], im2[:,:,1:2], im2[:,:,0:1], im3[:,:,2:3],im3[:,:,1:2]), axis=2)    #倒序读写cv,im3[:,:,0:1]为0通道
#         # im = np.concatenate((im1[:,:,0:1], im1[:,:,1:2], im1[:,:,2:3], im2[:,:,0:1], im2[:,:,1:2], im2[:,:,2:3], im3[:,:,0:1], im3[:,:,1:2]), axis=2)  #顺序读写jpeg4py，im3[:,:,2:3]为0通道
...
def hsijpg_loader(path):
    ...
    try:
        ...
        im2 = cv.imread(path.replace('img1', 'img2'), cv.IMREAD_COLOR)
        im3 = cv.imread(path.replace('img1', 'img3'), cv.IMREAD_COLOR)
        # Same 8-channel layout as former jpeg4py path: jpeg4py is RGB, slices were
        # (B,G,R) per frame then im3 (G,R); cv.imread is BGR so use [:,:,(0,1,2)],
        # (0,1,2), (1,2) respectively.
        im = np.concatenate((
            im1[:, :, 0:1], im1[:, :, 1:2], im1[:, :, 2:3], 
            im2[:, :, 0:1], im2[:, :, 1:2], im2[:, :, 2:3], 
            im3[:, :, 1:2], im3[:, :, 2:3]), axis=2)
