...
from lib.train.actors import SSTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "sstrack":
        net = build_sstrack(cfg)
    else:
    ...
    if settings.script_name == "sstrack":
        ...
        actor = SSTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
