...
Supports two log metric formats:
  - Multi-frame: ``0frame_Loss/total``, ``1frame_IoU``, ...
  - Single-frame (yxy): ``Loss/total``, ``IoU`` (normalized to frame 0)

Duplicate lines per (phase, epoch, iter) are averaged (e.g. multi-GPU prints).

Usage (requires conda env `must` for Python 3.9+ and matplotlib):

    conda activate must
    python scripts/plot_loss_yxy.py logs/yxy.log
    python scripts/plot_loss_yxy.py path/to/other.log -o logs/my_plots --smooth 100
"""
...
METRIC_RE_MULTI = re.compile(
    r"(\d+)frame_(Loss/(\w+)|IoU):\s*([\d.]+)"
)
METRIC_RE_SIMPLE = re.compile(
    r"(?:^|,\s*)(Loss/(\w+)|IoU):\s*([\d.]+)"
)


def _parse_metrics(line: str) -> dict:
    """Parse metrics into normalized keys: ``{frame}frame_Loss/{type}`` or ``{frame}frame_IoU``."""
    metrics = {}
    for fm in METRIC_RE_MULTI.finditer(line):
    ...
    for fm in METRIC_RE_SIMPLE.finditer(line):
        frame = 0
        if fm.group(1).startswith("Loss"):
            key = f"{frame}frame_Loss/{fm.group(2)}"
        else:
            key = f"{frame}frame_IoU"
        metrics[key] = float(fm.group(3))
    return metrics


def _dedupe_records(records):
    """Average metrics for duplicate (phase, epoch, iter) lines."""
    groups = defaultdict(list)
    for r in records:
        groups[(r["phase"], r["epoch"], r["iter"])].append(r)

    merged = []
    for rs in groups.values():
        all_keys = set().union(*(r["metrics"] for r in rs))
        metrics = {
            k: float(np.mean([r["metrics"][k] for r in rs if k in r["metrics"]]))
            for k in all_keys
        }
        base = rs[0]
        merged.append({**base, "metrics": metrics})
    merged.sort(key=lambda r: (r["phase"], r["epoch"], r["iter"]))
    return merged
...
def plot_all(records, out_dir: Path, log_name: str, smooth_win: int):
    ...
    saved = [
        "loss_epoch_total.png",
        "loss_train_iter.png",
        "loss_components.png",
        "iou_epoch.png",
    ]
    if len(frames) > 1:
        saved.extend(["loss_per_frame.png", "iou_val_per_frame.png", "iou_val_per_frame_grid.png"])
